...
var BedLinens = (function (_super) {
    ...
    function BedLinens(id, name, price, description, inStock, isSolidСolor, isBigSizes2, list) {
        var _this = _super.call(this, id, name, price, description, inStock) || this;
        _this.id = id;
        _this.name = name;
        _this.price = price;
    ...
    Basket.prototype.Add = function (val) {
        ...
        if (isNaN(num) || !((num ^ 0) === num) || num == 0 || productList[val].inStock < num) {
            if (productList[val].inStock < num)
                document.getElementById('modlalMessag').innerHTML = "Столько на складе нет. В наличии " + productList[val].inStock + " штук ";
            else
